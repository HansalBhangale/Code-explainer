...
async def get_symbol_type(symbol_id: str):
    ...
    try:
        ...
        if not type_info:
            raise HTTPException(
                status_code=404, detail="No type annotation found for this symbol"
            )

        return {"symbol_id": symbol_id, "type": type_info}
...
async def get_type_usage_stats(snapshot_id: str):
    ...
    try:
        ...
        return {"snapshot_id": snapshot_id, "type_count": len(stats), "types": stats}
    except Exception as e:
