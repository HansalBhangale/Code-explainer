...
class RepositoryLoader:
    ...
    def clone_git_repo(self, remote_url: str, repo_name: str) -> tuple[Repo, Path]:
        ...
        try:
            ...
            repo = Repo(
                name=repo_name, source_type=SourceType.GIT_REMOTE, remote_url=remote_url
            )
    ...
    def load_local_git_repo(
        self, local_path: str, repo_name: str = None
    ) -> tuple[Repo, Path]:
        ...
        try:
            ...
            logger.info(
                f"Loaded local repository {repo.name} at commit {commit_hash[:8]}"
            )
            return repo, path
    ...
    def load_directory(
        self, directory_path: str, repo_name: str = None
    ) -> tuple[Repo, Path]:
