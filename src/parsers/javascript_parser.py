...
class JavaScriptParser:
    ...
    def _extract_symbols(self, root: Node, source: str) -> List[Symbol]:
        ...
        def visit_node(node: Node, parent_class: Optional[str] = None):
            ...
            if node.type == "function_declaration":
                logger.debug("Found function_declaration node")
                ...
                if name_node:
                    ...
                    symbol = Symbol(
                        ...
                        meta={
                            "async": "async"
                            in source[node.start_byte : node.end_byte][:20]
                        },
                    )
                ...
                for child in node.children:
                    if child.type == "variable_declarator":
                        ...
                        if value_node and value_node.type == "call_expression":
                            ...
                            if (
                                func
                                and source[func.start_byte : func.end_byte] == "require"
                            ):
        ...
        def visit_node(node: Node):
            ...
            if node.type in ("function_declaration", "method_definition"):
                ...
                if name_node and return_type_node:
                    ...
                    if symbol_id:
                        type_name, category = self._parse_ts_type(
                            return_type_node, source
                        )
                        if type_name:
    ...
    def _get_function_signature(self, node: Node, source: str) -> str:
        ...
        try:
            ...
            params = (
                source[params_node.start_byte : params_node.end_byte]
                if params_node
                else "()"
            )
